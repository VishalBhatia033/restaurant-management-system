...
<header>
    ...
    <div class="filter" style='margin-top:2rem;align-items:center;'>
      <a href='manageorder.php' class="bt" >Pending Orders</a>
        <a href='confirmed_orders.php' class="bt" >Confirmed Orders</a>
        <a href='dispatched_orders.php' class="bt" >Dispatched Orders</a>
        <a href='picked.php' class="bt">Picked Orders</a>
        <a href='delivered_orders.php' class="bt"  style='background-color:rgb(60, 141, 188);color:white;'>Delivered Orders</a>
        <a href='cancled_orders.php' class="bt">Cancled Orders</a>
    </div>
    <div style='display:flex;justify-content:center; padding:1.5rem 0;padding-top:3rem;'>
            <form method="POST" action="delivered_orders.php" class='' style='width:93%;display:flex;gap:2rem;
            align-items:center;'>
        ...
        <h2 class='mt-5' style='margin:0;font-size:2rem;font-weight:600;text-align:center;margin:2rem 0;'>Delivered orders</h2>

        <?php
        if(isset($_POST['filter'])){
          $from = date("Y-m-d", strtotime($_POST['from']));
          $to = date("Y-m-d", strtotime($_POST['to']));
          $sql="SELECT * FROM orders WHERE createdat BETWEEN '$from' AND  '$to' AND orderstatus='delivered' AND cancle_status='0'  ORDER by id DESC";
        }else{
          $sql = "SELECT * FROM orders WHERE orderstatus='delivered' AND cancle_status='0'"; 
        $sql.= " GROUP BY orderid";
        
        }
        if(isset($_POST['id_search'])){
          $order_id=$_POST['order_id'];
          $sql="SELECT * FROM orders WHERE orderid='$order_id' AND orderstatus='delivered' AND cancle_status='0'";
        }else{
          $sql = "SELECT * FROM orders WHERE orderstatus='delivered' AND cancle_status='0'";
        $sql.= " GROUP BY orderid";
        }
...
if ($result->num_rows > 0) {
    echo "<div class='table_container' ><table  style='margin-bottom:4rem;'  >
       <tr>
       <th>S.No.</th>
       <th>Item</th>
       <th>User Id</th>
       ...
       while ($row = $result->fetch_assoc()) {
        ...
        // $s=1;
        echo"<tr>
        <td colspan='9' class='text-danger' style='font-weight:bold;'>Order ".$k++."</td>
        </tr>";
...
<td class='fw-bold'><span class='screen_cell'>" . $row2['orderstatus'] . "</span></td>
<td class='fw-bold'>" . $row2['picked_by'] . "</td>
<tr>";
