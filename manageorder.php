    ...
    <main style='display:flex;'>
        ...
        <div class="container-fluid" style="width:96%">
            <div class="filter" style='margin-top:2rem;align-items:center;'>
                <a href='manageorder.php' class="bt" style='background-color:rgb(60, 141, 188);color:white;'>Pending
                    Orders</a>
...
if(isset($_POST['filter'])){
    ...
    $sql="SELECT * FROM orders WHERE orderid= BETWEEN '$from' AND  '$to' AND orderstatus='pending' AND cancle_status='0'  ORDER by id DESC";
  }else{
    $sql = "SELECT * FROM orders WHERE orderstatus='pending' AND cancle_status='0'";
$sql.= " GROUP BY orderid";
  }

  if(isset($_POST['id_search'])){
    $order_id=$_POST['order_id'];
    $sql="SELECT * FROM orders WHERE orderid='$order_id' AND orderstatus='pending' AND cancle_status='0'";
  }else{
    $sql = "SELECT * FROM orders WHERE orderstatus='pending' AND cancle_status='0'";
$sql.= " GROUP BY orderid";
  }
...
<td>" . "<form action='' method='post'>" .
            ...
            '<select name="orderstatus" >
                <option value="pending">'.$row2['orderstatus'].'</option>
