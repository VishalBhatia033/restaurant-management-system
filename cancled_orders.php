...
if (isset($_POST['refund'])) {
    $ref=1;
    $s='Refund Complete';
    $updateSql = "UPDATE orders SET status = ? , refund=? WHERE orderid = ? AND username=?";
    $updateStmt2 = $conn->prepare($updateSql);
    $updateStmt2->bind_param("siss",$s,$ref, $_POST['orderid'],$_POST['username']);
    $updateStmt2->execute();
    ...
    <main style='display:flex;'>
        ...
        <div class="container-fluid" style="width:96%">
            <div class="filter" style='margin-top:2rem;align-items:center;'>
                <a href='manageorder.php' class="bt">Pending
                    Orders</a>
                ...
                <a href='cancled_orders.php' class="bt" style='background-color:rgb(60, 141, 188);color:white;'>Cancled
                    Orders</a>
            </div>


            <div style='display:flex;justify-content:center; padding:1.5rem 0;padding-top:3rem;'>
                <form method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>" class='' style='width:93%;display:flex;gap:2rem;
            align-items:center;'>
            ...
            <h2 class='mt-4' style='margin:0;font-size:2rem;font-weight:600;text-align:center;margin:2rem 0;'>Pending
                Orders</h2>
            <?php
...
if(isset($_POST['filter'])){
    ...
    $sql="SELECT * FROM orders WHERE orderid= BETWEEN '$from' AND  '$to' AND   cancle_status='1'  ORDER by id DESC";
  }else{
    $sql = "SELECT * FROM orders WHERE  cancle_status='1'";
$sql.= " GROUP BY orderid";
  }

  if(isset($_POST['id_search'])){
    $order_id=$_POST['order_id'];
    $sql="SELECT * FROM orders WHERE orderid='$order_id' AND  cancle_status='1'";
  }else{
    $sql = "SELECT * FROM orders WHERE   cancle_status='1'";
$sql.= " GROUP BY orderid";
  }
...
<input type='hidden' name='username' value='".$row2['username']."'/>
            <button ".$dis." type='submit' class='btn btn-sm btn-success' value='change' name='refund'>
               ".$txt."
            </button>"
            . "</form>" .
            "</td>
